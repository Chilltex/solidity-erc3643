...
abstract contract ExchangeMonthlyLimits is BasicCompliance {
    ...
    mapping(address => mapping(address => ExchangeTransferCounter))
    ...
    function addExchangeID(address _exchangeID) public onlyOwner {
        require(
            ...
            "ONCHAINID already tagged as exchange"
        );
    ...
    function complianceCheckOnExchangeMonthlyLimits(
        ...
        if (!isTokenAgent(_from) && _from != address(0)) {
            if (isExchangeID(receiverIdentity)) {
                ...
                if (
                    !_isExchangeMonthFinished(
                        receiverIdentity,
                        senderIdentity
                    ) &&
                    (
                        (getMonthlyCounter(receiverIdentity, senderIdentity) +
                            _value >
     ...
     */
    function _transferActionOnExchangeMonthlyLimits(
        address _from,
        address _to,
        uint256 _value
    ) internal {
    ...
    function _increaseExchangeCounters(
        ...
        if (
            (getMonthlyCounter(_exchangeID, _investorID) + _value) <=
            _exchangeMonthlyLimit[_exchangeID]
        ) {
            (_exchangeCounters[_exchangeID][_investorID])
                .monthlyCount += _value;
        }
    }
    ...
    function _resetExchangeMonthlyCooldown(
        ...
        if (_isExchangeMonthFinished(_exchangeID, _investorID)) {
            (_exchangeCounters[_exchangeID][_investorID]).monthlyTimer =
                block.timestamp +
                30 days;
