...
abstract contract BasicCompliance is AgentRole, ICompliance {
    ...
    modifier onlyAdmin() {
        require(
            owner() == msg.sender ||
                (AgentRole(address(tokenBound))).isAgent(msg.sender),
    ...
    function addTokenAgent(address _agentAddress) external override onlyOwner {
        require(
            !_tokenAgentsList[_agentAddress],
    ...
    function removeTokenAgent(
        ...
        require(
            _tokenAgentsList[_agentAddress],
            "This Agent is not registered yet"
        );
    ...
    function bindToken(address _token) external override {
        require(
            owner() == msg.sender ||
                (address(tokenBound) == address(0) && msg.sender == _token),
    ...
    function isTokenAgent(
        ...
        if (
            !_tokenAgentsList[_agentAddress] &&
            !(AgentRole(address(tokenBound))).isAgent(_agentAddress)
        ) {
            return false;
        }
        return true;
