      ...
      it("Should revert", async () => {
        ...
        await expect(
          trexImplementationAuthority
            ...
            .setTREXFactory(ethers.ZeroAddress)
      ...
      describe("When the authority has reference status true", () => {
        describe("When the trex factory to add is not using this authority contract", () => {
          it("Should revert for invalid link between the factory and the authority", async () => {
            ...
            const contractsStruct = {
              tokenImplementation:
                await implementations.tokenImplementation.getAddress(),
              ctrImplementation:
                await implementations.claimTopicsRegistryImplementation.getAddress(),
              irImplementation:
                await implementations.identityRegistryImplementation.getAddress(),
              irsImplementation:
                await implementations.identityRegistryStorageImplementation.getAddress(),
              tirImplementation:
                await implementations.trustedIssuersRegistryImplementation.getAddress(),
              mcImplementation:
                await implementations.modularComplianceImplementation.getAddress(),
            };
        ...
        describe("When the trex factory to add is using this authority contract", () => {
          it("should set the trex factory address", async () => {
            ...
            expect(await trexImplementationAuthority.getTREXFactory()).to.equal(
              await trexFactory.getAddress()
            );
          });
        });
    ...
    describe("When not called by the owner", () => {
      it("Should revert", async () => {
        ...
        await expect(
          trexImplementationAuthority
            .connect(anotherWallet)
            .setIAFactory(ethers.ZeroAddress)
        ).to.be.revertedWith("Ownable: caller is not the owner");
      });
    });
        ...
        describe("When the trex factory to add is using this authority contract", () => {
          it("should set the trex factory address", async () => {
            ...
            const tx = await trexImplementationAuthority.setIAFactory(
              await implementationAuthorityFactory.getAddress()
            );
            await expect(tx)
      ...
      describe("when caller is owner of every contract of the suite of the token", () => {
        ...
        describe("when the new implementation authority is not a reference contract and is not valid", () => {
          it("should revert", async () => {
            ...
            await otherTrexImplementationAuthority.useTREXVersion({
              major: 4,
              minor: 0,
              patch: 0,
            });
