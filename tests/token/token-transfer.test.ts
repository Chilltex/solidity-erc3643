...
describe("Token - Transfers", () => {
  describe(".approve()", () => {
    it("should approve a contract to spend a certain amount of tokens", async () => {
      ...
      expect(
        await token.allowance(aliceWallet.address, anotherWallet.address)
      ).to.equal(100);
    });
  });
  ...
  describe(".transfer()", () => {
    ...
    describe("when the sender has not enough balance", () => {
      it("should revert", async () => {
        ...
        await expect(
          token
            .connect(aliceWallet)
            .transfer(bobWallet.address, balance + BigInt(1000))
    ...
    describe("when the sender has not enough balance unfrozen", () => {
      it("should revert", async () => {
        ...
        await token
          .connect(tokenAgent)
          .freezePartialTokens(aliceWallet.address, balance - BigInt(100));
    ...
    describe("when amount is greater than unfrozen balance", () => {
      it("should unfroze tokens", async () => {
        ...
        await expect(tx)
          ...
          .withArgs(aliceWallet.address, balance - BigInt(150));
        expect(await token.getFrozenTokens(aliceWallet.address)).to.be.equal(
          50
        );
      });
    });
  ...
  describe(".burn()", () => {
    ...
    describe("when source wallet has not enough balance", () => {
      it("should revert", async () => {
        ...
        await expect(
          token
            .connect(tokenAgent)
            .burn(aliceWallet.address, balance + BigInt(1000))
    ...
    describe("when amount to burn is greater that unfrozen balance", () => {
      it("should burn and decrease frozen balance", async () => {
        ...
        const tx = await token
          .connect(tokenAgent)
          .burn(aliceWallet.address, balance - BigInt(50));
        await expect(tx)
          .to.emit(token, "Transfer")
          .withArgs(
            aliceWallet.address,
