...
export async function deployFullSuiteFixture() {
  ...
  await identityFactory
    .connect(deployer)
    // @ts-ignore
    // This method sources from onchain-id/solidity, which is not typed completely
  ...
  const claimTopicsRegistry = await ethers
    ...
    .then(async (proxy) =>
      ethers.getContractAt("ClaimTopicsRegistry", await proxy.getAddress())
    );
  ...
  const trustedIssuersRegistry = await ethers
    ...
    .then(async (proxy) =>
      ethers.getContractAt("TrustedIssuersRegistry", await proxy.getAddress())
    );
  ...
  const identityRegistryStorage = await ethers
    ...
    .then(async (proxy) =>
      ethers.getContractAt("IdentityRegistryStorage", await proxy.getAddress())
    );
  ...
  const claimForBob = {
    data: ethers.getBytes(ethers.toUtf8Bytes("Some claim public data.")),
    issuer: await claimIssuerContract.getAddress(),
    topic: claimTopics[0],
    scheme: 1,
